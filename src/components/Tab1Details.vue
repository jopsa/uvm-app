  ...
  <div>
    <ion-header>
      <ion-toolbar class="toolbar-style">
        ...
        <ion-title class="course-title">{{ this.courseDetails.name }} </ion-title>
      </ion-toolbar>
      <ion-progress-bar
        class="progress-bar"
        color="secondary"
    ...
    <ion-content padding class="dark-color-background">
      <v-flex xs12 sm12 class="content">
        <div>
          ...
          <div class="theory-text" v-if="showQuiz === false" v-html="breakIt(actualChapter.theory)"></div>

          <!-- quiz text -->
          <div v-if="showQuiz === true && seeResults === false">
            <div v-for="(question, index) in courseDetails.questions" :key="index">
          ...
          <div v-if="seeResults === true">
            <div class="header-results">
              <v-progress-circular :rotate="360" :size="100" :width="15" :value="resultPercentage" color="teal">
                {{ resultPercentage }}
              </v-progress-circular>
            ...
            <div>
              <div v-for="(question, index3) in wrongQuestions" :key="index3">
                ...
                <div class="error-color">
                  ...
                  {{ courseDetails.questions[question[0]].answers[question[1]].answer }}
                </div>
    ...
    <ion-tab-bar class="actions-bar" slot="bottom">
      ...
      <ion-tab-button class="back-button" @click="backChapter()" v-if="showBackButton"> &#60; </ion-tab-button>
      <ion-tab-button class="next-button" @click="nextChapter()" v-if="showNextButton"> > </ion-tab-button>
      <ion-tab-button class="quiz-button" @click="startQuiz()" v-if="showQuizButton"> Empezar Evaluación </ion-tab-button>
      <ion-tab-button class="see-results-button" @click="calculateResults()" v-if="showCalculateResultsButton">
        Ver resultado
      </ion-tab-button>
      <ion-tab-button class="go-back-button" @click="goToHome()" v-if="showGoBackToCategoriesButton">
  ...
  export default {
    ...
    methods: {
      getCorrectAnswerForQuestion(questionNumber) {
        return this.courseDetails.questions[questionNumber].answers.filter((answer) => answer.correct === true)[0]
          .answer;
      },
      calculateResults() {
        let correct = 0;
        ...
        this.answers.forEach((answer, indexQuestion) => {
          if (this.courseDetails.questions[indexQuestion].answers[answer].correct === true) {
            correct = correct + 1;
          } else {
      ...
      breakIt(value) {
        ...
        let subtitleCheck = subtitleCheckForBr1.split('<i>').join('<i style="color:#57b3ed;">');
        let subtitleCheckForBr2 = subtitleCheck.split('</i>').join('</i><br>');
        let brCheck = subtitleCheckForBr2.split('.').join('.<br>');
        return brCheck;
      },
      goToHome() {
        this.$router.push({ name: 'dashboard-details' });
      },
    },
  };
</script>
