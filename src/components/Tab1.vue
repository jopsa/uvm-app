  ...
  <ion-content class="dark-color-background">
    ...
    <ion-text v-if="userDetails">
      <div class="user-infos">
        ...
        <a href="#" v-if="user" @click="signOut">Cerrar sesión</a>
      </div>
    </ion-text>
    <ion-card class="statistic-card" v-if="userDetails">
      <ion-grid>
        <ion-row class="statistics">
          <ion-item lines="none" class="statistic-item">{{ userDetails.courses.length }} evaluaciones completas </ion-item>
          <div class="statistic-item">&#62;</div>
          <ion-item lines="none" class="statistic-item">{{ getUserTotalPoints }} puntos </ion-item>
        </ion-row>
      </ion-grid>
    </ion-card>
          ...
          <ion-slide class="slider" v-for="course in category.courses" :key="course.id">
            <div class="slide-content" @click="viewCourse(course)">
              ...
              <div class="slider-chapters" v-if="course && course.chapters">
                Capitulos: {{ course.chapters.length }}
              </div>
            </div>
          </ion-slide>
        </ion-slides>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getUserTotalPoints() {
        if (this.userDetails && this.userDetails.courses && this.userDetails.courses.length > 0) {
          return this.userDetails.courses.reduce((acc, currVal) => acc + currVal, 0);
        } else {
          return 0;
        }
      },
    ...
    methods: {
      viewCourse(course) {
        this.$router.push({ name: 'dashboard-details', params: { course } });
      },
